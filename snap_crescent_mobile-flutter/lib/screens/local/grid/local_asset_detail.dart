...
class LocalAssetDetailScreen extends StatelessWidget {
  ...
  final LocalAssetsDetailArguments _arguments;
...
class _LocalPhotoDetailViewState extends State<_LocalPhotoDetailView> {
  ...
  Widget build(BuildContext context) {
    ...
    _uploadAsset(int assetIndex, BuildContext context) async {
      ToastService.showSuccess((widget.type == ViewType.PHOTO ? "Photo" : "Video") + " upload in Progress");
      final File? assetFile = await _getAssetFile(assetIndex);

      if(widget.type == ViewType.PHOTO) {
        await PhotoService().save(assetFile!);
      } else{
        await VideoService().save(assetFile!);
      }
      
      ToastService.showSuccess((widget.type == ViewType.PHOTO ? "Photo" : "Video") + " uploaded successfully");
    }
      ...
      return FutureBuilder<File?>(
          ...
          builder: (BuildContext context, AsyncSnapshot<File?> snapshot) {
            if (snapshot.data == null) {
              ...
              return widget.type == ViewType.PHOTO ? _imageBanner(snapshot.data) : _videoPlayer(snapshot.data);
            }
          });
    }
    ...
    _pageView() {
      return Container(
        ...
        child: PageView.builder(
          ...
          physics: widget.type == ViewType.PHOTO ?  PageScrollPhysics() : NeverScrollableScrollPhysics(),
