...
class AssetState {
  ...
  List<int> getSelectedIndexes() {
    return assetList.where((asset) => asset.selected == true).map((asset) => assetList.indexOf(asset)).toList();
  }

  bool isAnyItemSelected() {
    return assetList.firstWhereOrNull((asset) => asset.selected == true) != null;
  }
  ...
  void addAsset(UniFiedAsset asset) {
    ...
    if (groupedAssets.containsKey(key)) {
      ...
      for (var unifiedAsset in unifiedAssets) {
        if (asset.assetSource == AssetSource.DEVICE && asset.assetSource == unifiedAsset.assetSource) {
          if (unifiedAsset.assetEntity!.id == asset.assetEntity!.id) {
            assetAlreadyPresent = true;
            continue;
          }
        } else if (asset.assetSource == AssetSource.CLOUD && asset.assetSource == unifiedAsset.assetSource) {
  ...
  void prepareGroupedMapKeysList() {
    List<DateTime> dateTimeKeys = groupedAssets.keys.toList().map((key) => Constants.defaultYearFormatter.parse(key)).toList();
