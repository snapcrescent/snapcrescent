...
class AssetService extends BaseService {
  ...
  Future<BaseResponseBean<int, Asset>> search(AssetSearchCriteria searchCriteria) async {
    try {
      if (await super.isUserLoggedIn()) {
        Dio dio = await getDio();
        Options options = await getHeaders();
        final response = await dio.post('/asset/sync', data: searchCriteria.toJson(), options: options);
  ...
  Future<List<Asset>> searchAndSyncInactiveRecords(AssetSearchCriteria searchCriteria) async {
  ...
  Future<dynamic> saveOnLocal(Asset entity, bool createIfNotFound, List<Future> thumbnailDownloaderJobs) async {
    ...
    if (assetExistsById == false) {
      final thumbnailExistsById = await ThumbnailRepository().existsById(entity.thumbnailId!);
      ...
      final assetMetadataExistsById = await MetadataRepository().existsById(entity.metadataId!);
  ...
  Future<List<int>> assetIdsOnLocal() async {
    return AssetRepository().findAllIds();
  }

  Future<int> countOnLocal() async {
    return AssetRepository().countOnLocal(AssetSearchCriteria.defaultCriteria());
  }
  ...
  Future<void> deleteUploadedAssets(DateTime tillDate) async {
    ...
    if (localAssetsSyncedWithServer != null && localAssetsSyncedWithServer.isNotEmpty) {
      ...
      List<String> syncedAssetIds = localAssetsSyncedWithServer.map((metadata) => metadata.localAssetId!).toList();
  ...
  _getAssetFile(List<int> assetIndexes) async {
    ...
    for (var assetIndex in assetIndexes) {
      ...
      if (unifiedAsset.assetSource == AssetSource.CLOUD) {
        Asset asset = unifiedAsset.asset!;
        assetFile = await tempDownloadAssetById(asset.id!, asset.metadata!.name!, unifiedAsset.assetType);
      } else {
  ...
  Future<bool> uploadAssetFilesToServer(List<int> assetIndexes) async {
    for (var assetIndex in assetIndexes) {
      ...
      if (unifiedAsset.assetSource == AssetSource.DEVICE) {
        ...
        Metadata? metadata = await MetadataRepository().findByNameAndSize(fileName, assetFile.lengthSync());
